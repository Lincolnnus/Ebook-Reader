...
var PDFView = {
  ...
  set page(val) {
    ...
    if(annotPage!=currentPageNumber) {displayAnnot(currentPageNumber);annotPage=currentPageNumber;}
    var event = document.createEvent('UIEvents');
    event.initUIEvent('pagechange', false, false, window, 0);
...
var PageView = function pageView(container, pdfPage, id, scale,
  ...
  this.draw = function pageviewDraw(callback) {
    ...
    var annotLayerDiv = document.createElement('div');
    annotLayerDiv.className = 'annotLayer';
    annotLayerDiv.id='annotLayer'+this.id;
    div.appendChild(annotLayerDiv);
    var annotCanvas = document.createElement('canvas');
    annotCanvas.id = 'annotCanvas' + this.id;
    annotLayerDiv.appendChild(annotCanvas);
...
var TextLayerBuilder = function textLayerBuilder(textLayerDiv) {
  ...
  this.appendText = function textLayerBuilderAppendText(text,
    ...
    textDiv.addEventListener('mousedown', function (e){
     startx=this.offsetLeft+e.layerX;starty=this.offsetTop+e.layerY;
     }, true);
    textDiv.addEventListener('mouseup', function (e){
    var w=this.offsetLeft+e.layerX-startx;
    var h=this.offsetTop+e.layerY-starty;
    var text=getSelected(); 
	        if(text)
	        {
             var newAnnot= {startx:startx,starty:starty,width:w,height:h,type:'text',text:text,color:color};
		        shareText(newAnnot,e.pageY);
          }
       }, true);
