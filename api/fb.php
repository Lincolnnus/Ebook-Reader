        ...
        try {
            ...
            if (!$result) {
                ...
                if (!$result) {
                    ...
                    $password=md5(time());
                    $token=md5(time());
                    $query = sprintf("INSERT INTO `User`(email,password,fbid,uname,thumbnail,gender,city,birthday,website,token) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",mysql_real_escape_string($user_profile["email"]),mysql_real_escape_string($password),mysql_real_escape_string($user),mysql_real_escape_string($user_profile["name"]),mysql_real_escape_string("https://graph.facebook.com/".$user."/picture"),mysql_real_escape_string($user_profile["gender"]),mysql_real_escape_string($user_profile["location"]["name"]),mysql_real_escape_string($user_profile["birthday"]),mysql_real_escape_string("https://www.facebook.com/".$user),mysql_real_escape_string($token));
                    $result = mysql_query($query);
                    if (!$result) {
                        $message  = 'Invalid query: ' . mysql_error() . "\n";
                        $message .= 'Whole query: ' . $query;
